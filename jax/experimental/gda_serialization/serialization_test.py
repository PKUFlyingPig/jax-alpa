...
from jax._src.lib.xla_bridge import process_index
...
class CheckpointTest(jtu.JaxTestCase):
  def setUp(self):
    super().setUp()
    jax.distributed.initialize(flags.FLAGS.server_addr, flags.FLAGS.num_hosts, flags.FLAGS.host_idx)
  ...
  def test_checkpointing(self):
    ...
    if jax.process_index() == 0:
      ...
      for i, s in enumerate(m3.local_shards):
        self.assertEqual(s.index, (slice(None),))
        self.assertEqual(s.replica_id, i + 4)
        self.assertArraysEqual(s.data.to_py(), np.array([]))
        self.assertEqual(m3.dtype, np.float32)
